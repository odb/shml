...
fake_install_yes() {
	echo -e "\nCreating Coversheet for TPS Reports... ${cs_f[green]}${cs_checkmark} Pass${cs_f[end]}"
	sleep 1;
	echo -e "Misplaced decimal check... ${cs_f[red]}${cs_xmark} Fail${cs_f[end]}"
	sleep 1;
	echo -e "Divert fractions of pennies into a bank account... ${cs_f[green]}${cs_checkmark} Pass${cs_f[end]}"
	sleep 1;
	echo -e "${cs_f[green]}${cs_checkmark} Program installed${cs_f[end]}"
	sleep 1;
	echo -e "Account Balance: ${cs_a[bold]}${cs_f[red]}\$305,326.13 ${cs_f[end]}${cs_a[end]}"
	sleep 1;
	echo -e "${cs_hr}${cs_br}"
	echo -e "${cs_a[bold]}${cs_f[red]}CALL MICHAEL! ${cs_f[end]}${cs_a[end]}"
}
...
while true; do
    ...
    case $yn in
        ...
        [Nn]* ) exit;;
        * ) echo -e "Please answer ${cs_b[green]}yes${cs_b[end]} or ${cs_b[red]}no${cs_b[end]}.";;
    esac
done
